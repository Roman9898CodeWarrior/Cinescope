...
from API.api_tests_data.payment_data import PaymentData
from API.api_tests_data.user_data import UserDataForApiTests
from API.entities.user import CommonUser
from API.entities.user import AdminUser
from API.api_tests_models.payment_data_model import DataForPaymentCreationModel
...
def common_user_registered(user_session, fixture_data_for_user_registration):
    with allure.step('Регистрация пользователя и аутентификация с его кредами.'):
        ...
        register_common_user_response_validated = new_session.auth_api.register_user(fixture_data_for_user_registration)
...
def fixture_data_with_non_valid_password_for_user_registration():
    return UserDataForApiTests.get_non_valid_user_data_for_registration()
...
def fixture_data_for_user_creation_by_admin():
    def _fixture_data_for_user_creation_by_admin():
        return UserDataForApiTests.get_user_data_for_creation_by_admin()
...
def fixture_test_user_created_by_admin_changed_data():
    return UserDataForApiTests.get_user_data_for_change_by_admin()
