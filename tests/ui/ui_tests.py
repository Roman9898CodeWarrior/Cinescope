...
def test_demo_qa_check_visibility(page):
    ...
    #assert page.is_visible('span:has-test("Desktop")') == False

    page.get_by_title('Toggle').click()
    #expect(page.locator('span:has-text("Desktop")')).to_be_visible()
    expect(page.get_by_text('Desktop')).to_be_visible()
    # assert page.is_visible('span:has-text("Desktop")') == True
    #page.wait_for_selector('span:has-text("Desktop")', state='visible', timeout=5000)

    #page.pause()
    page.get_by_title('Toggle').nth(2).click()
